...
for idx, (mouse, pos, time) in enumerate(zip(mouse_names, all_positions, all_times)):
    ...
    if idx == 0:
        ...
        start = 1000
        finish = 1000+ 250*20
    axes1[idx].plot(pos[start:finish], color='tab:blue')
    axes1[idx].set_ylabel('Position')
    axes1[idx].set_title(f'{mouse} - Position over Time')
    axes1[idx].grid(False)
axes1[-1].set_xlabel('Time')
plt.tight_layout()
plt.savefig(os.path.join(figures_directory, 'Position_over_Time_all_mice.svg'))
plt.show()
# === PLOT: SPEED OVER TIME ===
fig2, axes2 = plt.subplots(len(mouse_names), 1, figsize=(12, 3 * len(mouse_names)))
if len(mouse_names) == 1:
...
for idx, (mouse, speed, time) in enumerate(zip(mouse_names, all_speeds, all_times)):
    if idx == 0:
        ...
        start = 1000
        finish = 1000+ 250*20
    axes2[idx].plot(speed[start:finish], color='tab:orange')
    axes2[idx].set_ylabel('Speed')
    axes2[idx].set_title(f'{mouse} - Speed over Time')
    axes2[idx].grid(False)
axes2[-1].set_xlabel('Time')
plt.tight_layout()
plt.savefig(os.path.join(figures_directory, 'Speed_over_Time_all_mice.svg'))
plt.show()