    ...
    for session_index in sessions[rat_index]:
        ...
        for probe in stimes.keys():
            ...
            for neuron in range(len(spikes_probe)):
                ...
                maze_spikes_index = np.logical_and(neuron_spikes > positionTimeStamps[0] * 20000, neuron_spikes < positionTimeStamps[-1]* 20000 )
                ...
                valid_indices = indices[(indices >= 0) & (indices < len(positionTimeStamps))]
                # Count the occurrences of each index in valid_indices (number of spikes per bin)
                counts = np.bincount(valid_indices, minlength=len(positionTimeStamps))
                # Assign counts to the appropriate column for the current neuron
                spike_matrix[:, neuron] = counts
            ...
            output_dic['timeStamps'] = positionTimeStamps
